...
module.exports = async function (context, req) {
  try {
    ...
    // Expect the Teams card to send the ticket number in "ticketId"
    ...
    // Generate the current timestamp in the expected format (local system time)
    ...
    // Remove any leading letters and zeros.
    // For example, "EPM000238" becomes "238"
    ...
    // - decisionTime: currentTime (local system time)
    // - message: "handled in teams"
    // - systemId: same as requestId
    // - ticketId: as provided from the Teams card (e.g. "EPM000238")
    // - ticketUrl: constructed from the ticketId (with leading letters and zeros stripped)
    const payload = {
    ...
    // Note: Do not change the approval endpoint below.
